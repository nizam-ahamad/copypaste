...
module.exports.prototype = {
  ...
  __construct: function (sToken, nTokenLifetime) {
    ...
    // 2. register
    this._elRoot = document.querySelector(
      '[data-mimoto-id="component_Connector"]',
    );
    this._elCardFront = this._elRoot.querySelector(
      '[data-mimoto-id="card_front"]',
    );
    this._elCardBack = this._elRoot.querySelector(
      '[data-mimoto-id="card_back"]',
    );
    this._elConnectionTypesContainer = this._elRoot.querySelector(
      '[data-mimoto-id="connectiontypes-container"]',
    );

    // 3. register - connection types
    this._elConnectionTypeScan = this._elConnectionTypesContainer.querySelector(
      '[data-mimoto-id="type_scan"]',
    );
    this._elConnectionTypeManually =
      this._elConnectionTypesContainer.querySelector(
        '[data-mimoto-id="type_manually"]',
      );
    this._elConnectionTypeInvite =
      this._elConnectionTypesContainer.querySelector(
        '[data-mimoto-id="type_invite"]',
      );

    // 4. store
    this._aConnectionTypes[ConnectionTypes.prototype.TYPE_SCAN] =
      this._elConnectionTypeScan;
    this._aConnectionTypes[ConnectionTypes.prototype.TYPE_MANUALLY] =
      this._elConnectionTypeManually;
    this._aConnectionTypes[ConnectionTypes.prototype.TYPE_INVITE] =
      this._elConnectionTypeInvite;
    ...
    this._elConnectionTypeScan_QRContainer =
      this._elConnectionTypeScan.querySelector('[data-mimoto-id="container"]');

    // --- connection type `manually`

    // 6. register
    this._elConnectionTypeManually_Instructions =
      this._elConnectionTypeManually.querySelector(
        '[data-mimoto-id="instructions"]',
      );
    this._elConnectionTypeManually_Code =
      this._elConnectionTypeManually.querySelector('[data-mimoto-id="code"]');
    this._elConnectionTypeManually_Countdown =
      this._elConnectionTypeManually.querySelector(
        '[data-mimoto-id="countdown"]',
      );
    this._elConnectionTypeManually_URL =
      this._elConnectionTypeManually.querySelector(
        '[data-mimoto-id="connect_url"]',
      );

    // 7. setup
    this._elConnectionTypeManually_URL.innerText =
    ...
    this._elConnectionTypeInvite_ButtonRefreshToken =
      this._elConnectionTypeInvite.querySelector(
        '[data-mimoto-id="button-refreshtoken"]',
      );
    this._elConnectionTypeInvite_TimeTillExpiration =
      this._elConnectionTypeInvite.querySelector(
        '[data-mimoto-id="timetillexpiration"]',
      );
    this._elConnectionTypeInvite_NotificationCopiedToClipboard =
      this._elConnectionTypeInvite.querySelector(
        '[data-mimoto-id="notification-copiedtoclipboard"]',
      );
    this._elConnectionTypeInvite_Options =
      this._elConnectionTypeInvite.querySelector(
        '[data-mimoto-id="inviteoptions"]',
      );
    this._elConnectionTypeInvite_ButtonWhatsapp =
      this._elConnectionTypeInvite_Options.querySelector(
        '[data-mimoto-id="button-whatsapp"]',
      );
    this._elConnectionTypeInvite_ButtonTelegram =
      this._elConnectionTypeInvite_Options.querySelector(
        '[data-mimoto-id="button-telegram"]',
      );
    this._elConnectionTypeInvite_ButtonEmail =
      this._elConnectionTypeInvite_Options.querySelector(
        '[data-mimoto-id="button-email"]',
      );
    this._elConnectionTypeInvite_ButtonCopyLink =
      this._elConnectionTypeInvite_Options.querySelector(
        '[data-mimoto-id="button-copylink"]',
      );

    // 9. configure
    if (this._elConnectionTypeInvite_ButtonCopyLink)
      this._elConnectionTypeInvite_ButtonCopyLink.addEventListener(
        "click",
        this._onButtonCopyLinkClick.bind(this),
      );
    if (this._elConnectionTypeInvite_ButtonRefreshToken)
      this._elConnectionTypeInvite_ButtonRefreshToken.addEventListener(
        "click",
        this._onButtonRefreshTokenClick.bind(this),
      );
    ...
    // 12. show
    this._elCardFront.appendChild(
      this._aConnectionTypes[this._sCurrentConnectionType],
    );
    ...
    // 14. configure
    this._menuConnectionType.addEventListener(
      MenuConnectionType.prototype.REQUEST_TOGGLE_CONNECTIONTYPE,
      this._onRequestToggleConnectionType.bind(this),
    );
  },
  ...
  show: function () {
    ...
    // 3. position
    this._elCardFront.style.left =
      -Math.floor(this._elCardFront.offsetWidth) / 2 + "px";
  ...
  hide: function () {
    ...
    if (this._timerManualCodeCountdown)
  ...
  setToken: function (sToken, nTokenLifetime) {
    ...
    // 2. compose
    this._sTokenURL =
      window.location.protocol + "//" + window.location.hostname + "/" + sToken;
    ...
    // 4. output
    if (this._elConnectionTypeInvite_ButtonWhatsapp)
      this._elConnectionTypeInvite_ButtonWhatsapp.setAttribute(
        "data-url",
        this._sTokenURL,
      );
    if (this._elConnectionTypeInvite_ButtonTelegram)
      this._elConnectionTypeInvite_ButtonTelegram.setAttribute(
        "data-url",
        this._sTokenURL,
      );
    if (this._elConnectionTypeInvite_ButtonEmail)
      this._elConnectionTypeInvite_ButtonEmail.setAttribute(
        "data-url",
        this._sTokenURL,
      );

    // 4. start
    this._timerTokenExpires = setTimeout(
      this._onTimerTokenExpires.bind(this),
      nTokenLifetime,
    );
    this._timerTokenUpdate = setInterval(
      this._onTimerTokenUpdate.bind(this),
      1000,
    );
  ...
  setManualCode: function (sToken, nTokenLifetime) {
    ...
    // 2. output
    this._elConnectionTypeManually_Code.innerText =
      sToken.substr(0, 3) + "-" + sToken.substr(3);

    // 3. setup
    this._manualCode.localCreated = new Date().getTime();
    this._manualCode.localExpires =
      this._manualCode.localCreated + nTokenLifetime;
    ...
    // 6. start
    this._timerManualCodeCountdown = setInterval(
      this._onTimerManualCodeCountdown.bind(this),
      100,
    );
  },
  ...
  _onRequestToggleConnectionType: function (sConnectionType) {
    ...
    if (this._bIsFrontCardFocused) {
      // a. cleanup
      while (this._elCardBack.children.length > 0)
        this._elCardBack.removeChild(this._elCardBack.children[0]);

      // b. show
      this._elCardBack.appendChild(this._aConnectionTypes[sConnectionType]);

      // c. position
      this._elCardBack.style.left =
        -Math.floor(this._elCardBack.offsetWidth) / 2 + "px";
    } else {
      // a. cleanup
      while (this._elCardFront.children.length > 0)
        this._elCardFront.removeChild(this._elCardFront.children[0]);
  ...
  _onTimerTokenExpires: function (bGetInviteToken) {
    ...
    // 2. request
    this.dispatchEvent(
      this.REQUEST_TOKEN_REFRESH,
      bGetInviteToken === true ? true : false,
    );
  },
  ...
  _onTimerTokenUpdate: function () {
    ...
    if (
      this._sCurrentConnectionType === ConnectionTypes.prototype.TYPE_INVITE &&
      nDifference < 3 * 60 * 1000
    ) {
      // a. auto refresh
      this._onTimerTokenExpires(true);
    ...
    // 3. convert
    let nMinutes =
      Math.round((10 * (nDifference % (1000 * 60 * 60))) / (1000 * 60)) / 10;

    // 4. output
    if (this._elConnectionTypeInvite_TimeTillExpiration)
      this._elConnectionTypeInvite_TimeTillExpiration.innerText =
        nMinutes + " mins";
  },
  ...
  _updateExpirationLabel: function () {
    ...
    let sRemainingTime = "";
    if (nMinutes > 0)
      sRemainingTime = nMinutes + " " + (nMinutes === 1 ? "min" : "mins") + " ";
    sRemainingTime +=
      (nSeconds === 60 ? (nMinutes !== 0 ? 0 : nSeconds) : nSeconds) +
      " " +
      (nSeconds === 1 ? "sec" : "secs");

    // 4. output
    this._elConnectionTypeManually_Countdown.innerText =
      "(valid for " + sRemainingTime + ")";
    ...
    if (nDifference <= 0) {
      ...
      // d. request new
      if (
        this._sCurrentConnectionType === ConnectionTypes.prototype.TYPE_MANUALLY
      )
  ...
  _onButtonCopyLinkClick: function () {
    ...
    // 2. style
    if (!this._elConnectionTypeInvite_NotificationCopiedToClipboard) return;
    this._elConnectionTypeInvite_NotificationCopiedToClipboard.classList.add(
      "show",
    );
    ...
    setTimeout(
      function () {
        // a. cleanup
        this._elConnectionTypeInvite_NotificationCopiedToClipboard.classList.remove(
          "show",
        );
      }.bind(this),
      2000,
