...
module.exports = {
  ...
  _collectStats: function () {
        ...
        .aggregate([
          {
            $group: {
              ...
              connectedCount: {
                $sum: { $cond: [{ $ne: ["$connected", false] }, 1, 0] },
              },
              connectionTypeScan: {
                $sum: {
                  $cond: [
              ...
              archivedCount: {
                $sum: { $cond: [{ $ne: ["$archived", false] }, 1, 0] },
              },
            },
          },
          {
        ...
        .toArray(
          function (err, aDocs) {
            ...
            let stats = {
              ...
              idle:
                result.pairCount - result.activeCount - result.archivedCount,
            ...
            // d. store
            if (false)
              this.Mimoto.mongoDB.getCollection("stats").insertOne(stats);
          }.bind(this),
        );
    ...
    //
    //
    // if (false) this.Mimoto.mongoDB.getCollection('pairs').aggregate(
    //     [
    //         // {"$project": {"logs":1}},
    //         { "$unwind": "$logs" },
    ...
    //
    //
    // // if (false) this.Mimoto.mongoDB.getCollection('pairs').find(
    // //     {
    // //         "logs.action": "DATA",
    // //         "archived": true
    // //     }
    ...
    // //         }
    // //         );
    // //     if (false) this.Mimoto.mongoDB.getCollection('pairs').bulkWrite(bulkArray, {ordered:true, w:1});
    // //
    // // }.bind(this));
    //
    //
