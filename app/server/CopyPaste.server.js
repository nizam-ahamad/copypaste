...
module.exports = {
  ...
  _config: {
    ...
    https: false, // options: 'true' (runs on https)  | 'false' (runs on http)
    mongo: false,
  ...
  _startupSocketIO: function () {
    ...
    if (this._config.https) {
      ...
      //   this._app.use(Module_Express.static("."));
      ...
      // This tells the server to look in the 'web' folder relative to where the script is
    ...
    // 4. listen
    this._server.listen(
      this._configFile.socketio.server.port,
      this._configFile.socketio.server.host,
      this._onSocketIOConnected.bind(this),
    );
  },
  ...
  _init: function () {
    ...
    this.Mimoto.logger = new Logger(
      this._configFile.logtofile.file
        ? this._configFile.logtofile.file.toString()
        : "",
  ...
  _onRequestPrimaryDeviceConnect: function (
    ...
    // 4. output
    this._logUsers(
      "Primary Device with socket.id = " + primaryDeviceSocket.id,
      "Requests token = " + token.getValue(),
    );
  },
  ...
  _onRequestDeviceReconnect: function (socket, sDeviceID, sPreviousSocketID) {
    ...
    if (sPreviousSocketID) {
      ...
      if (
        ...
        this.Mimoto.logger.log(
          "ALERT - The device DOES EXIST and seems to be the one we lost due to bad disconnect sDeviceID = " +
            sDeviceID,
        );
    ...
    if (!newDevice || !originalDevice) {
      // a. output
      this.Mimoto.logger.log(
        "ALERT - No original device after server restart or device offline - sDeviceID = " +
          sDeviceID +
          "\n\n",
      );
      ...
      let bMightHaveBeenUnableToLogOffEarlier =
        undisconnectedDevice && !sPreviousSocketID ? true : false;
    ...
    // 10. output
    this._logUsers(
      "Device `" + device.getType() + "` with sDeviceID = `" + sDeviceID + "`",
      "Reconnected to pair (socket.id = " + socket.id + ")",
    );
  },
  ...
  _onRequestSecondaryDeviceConnectByQR: function (
    ...
    if (token === false) {
      this.Mimoto.logger.log(
        "SECONDARYDEVICE_CONNECT_TOKEN_NOT_FOUND for sTokenValue=`" +
          sTokenValue +
          "` from socket.id=`" +
          socket.id +
          "`",
    ...
    // 8. output
    this._logUsers(
      "Secondary device with socket.id = " + socket.id,
      "Requests connection to token = " + token.getValue(),
    );
  },
  ...
  _onRequestPrimaryDeviceManualCode: function (socket) {
    ...
    // 5. output
    this.Mimoto.logger.log(
      "Socket.id = " + socket.id + " has requested manual code",
    );
  },
  ...
  _onRequestSecondaryDeviceConnectByManualCode: function (
    ...
    if (token === false) {
      this.Mimoto.logger.log(
        "ERROR_SECONDARYDEVICE_CONNECT_BY_MANUALCODE_TOKEN_NOT_FOUND for sManualCode=`" +
          sManualCode +
          "` from socket.id=`" +
          socket.id +
          "`",
    ...
    this._logUsers(
      "Secondary device with socket.id = " + socket.id,
      "Requests connection to manual code = " + token.getValue(),
    );
  },
  ...
  _onRequestPrimaryDeviceManualCodeConfirmed: function (socket) {
    ...
    if (!pair.confirmUnconfirmedSecondaryDevice()) {
      ...
      // b. output
      this._logUsers(
        "Secondary device NOt connected by manual code because it`s not there anymore",
      );
    ...
    // 6. output
    this._logUsers(
      "Secondary device connected by manual code (socket.id = " +
        pair.getSecondaryDevice().id +
        ")",
    );
  },
  ...
  _logUsers: function () {
    ...
    this.Mimoto.logger.log(
      "Devices (count = " +
        this.Mimoto.deviceManager.getNumberOfDevices() +
        ")",
    );
    this.Mimoto.logger.log("------------------------------------------");
    for (let sKey in aDevices) {
      this.Mimoto.logger.log(
        "device = ",
        this._prepareDeviceForOutput(aDevices[sKey]),
      );
    }
    this.Mimoto.logger.log("");
    ...
    this.Mimoto.logger.log(
      "Offline devices (count = " +
        this.Mimoto.deviceManager.getNumberOfOfflineDevices() +
        ")",
    );
    this.Mimoto.logger.log("------------------------------------------");
    for (let sKey in aOfflineDevices) {
      this.Mimoto.logger.log(
        "device =",
        this._prepareDeviceForOutput(aOfflineDevices[sKey].device),
        "Expires at " +
          Utils.prototype.buildDate(aOfflineDevices[sKey].nExpires),
      );
      this.Mimoto.logger.log("");
    }
    this.Mimoto.logger.log("");
    ...
    this.Mimoto.logger.log(
      "Pairs (count = " +
        this.Mimoto.pairManager.getNumberOfActivePairs() +
        ")",
    );
    this.Mimoto.logger.log("------------------------------------------");
    for (let sKey in aPairs) {
      this.Mimoto.logger.log(
        "pair =",
        this._preparePairForOutput(aPairs[sKey]),
        "State =  " + (aPairs[sKey].isActive() ? "active" : "idle"),
        !aPairs[sKey].isActive()
          ? "Expires at " +
              Utils.prototype.buildDate(aPairs[sKey].getExpiryDate())
          : "",
      );
      this.Mimoto.logger.log("");
    }
