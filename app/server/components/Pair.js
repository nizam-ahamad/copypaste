...
module.exports.prototype = {
  ...
  __construct: function (socket, sPublicKey, sDeviceID) {
    ...
    // 4. store
    if (this.Mimoto && this.Mimoto.mongoDB)
      this.Mimoto.mongoDB.getCollection("pairs").insertOne({
  ...
  reconnectPrimaryDevice: function (device) {
    // 1. validate
    if (
      this.hasPrimaryDevice() ||
      (this.getPrimaryDeviceID() &&
        this.getPrimaryDeviceID() !== device.getID())
    ) {
      // a. act
   ...
   */
  connectSecondaryDevice: function (
    socket,
    sPublicKey,
    device,
    sConnectionType,
    ...
    // 1. validate
    if (
      this.hasSecondaryDevice() ||
      (this.getSecondaryDeviceID() &&
        this.getSecondaryDeviceID() !== device.getID())
    ) {
      // a. act
    ...
    if (this.Mimoto && this.Mimoto.mongoDB)
      this.Mimoto.mongoDB.getCollection("pairs").updateOne(
        ...
        {
          $set: {
            connected: Utils.prototype.buildDate(),
  ...
  confirmUnconfirmedSecondaryDevice: function () {
    // 1. load
    let device = this.Mimoto.deviceManager.getDeviceByDeviceID(
      this._sSecondaryDeviceID,
    );
    ...
    // 3. convert
    this.connectSecondaryDevice(
      this._unconfirmedSecondaryDeviceSocket,
      this._sSecondaryDevicePublicKey,
      device,
  ...
  hasOtherDevice: function (socket) {
    // 1. verify
    if (this.hasPrimaryDevice() && this.getPrimaryDevice().id === socket.id)
      return this.hasSecondaryDevice();

    // 2. verify
    if (this.hasSecondaryDevice() && this.getSecondaryDevice().id === socket.id)
      return this.hasPrimaryDevice();
  ...
  toggleDirection: function () {
    ...
    this._sDirection =
      this._sDirection === ToggleDirectionStates.prototype.DEFAULT
        ? ToggleDirectionStates.prototype.SWAPPED
        : ToggleDirectionStates.prototype.DEFAULT;
  },
  ...
  sendData: function (encryptedData) {
    ...
    // 2. register
    let receiverSocket =
      this.getDirection() === ToggleDirectionStates.prototype.SWAPPED
        ? this.getSecondaryDevice()
    ...
    if (dataCloneForLogging.value.data.length > 20) {
      ...
      dataCloneForLogging.value.data =
        sValue.substr(0, 10) + " ... " + sValue.substr(sValue.length - 10);
    }
    ...
    if (bStateHasChanged) {
      ...
      if (this._bIsActive) {
        ...
        this._timerExpiration = setTimeout(
          this._onHandleExpiration.bind(this),
          this.MAX_IDLE_TIME,
        );
  ...
  _handlePossibleSecurityBreach: function (device, sDeviceLabel) {
    // 1. output
    if (this.Mimoto && this.Mimoto.logger) {
      this.Mimoto.logger.log(
        "Pair already has a " +
          sDeviceLabel +
          " device connected. sPairID = " +
          this.getID(),
      );
    }
    ...
    // 2. broadcast
    this.dispatchEvent(
      ConnectorEvents.prototype.ERROR_SECURITY_COMPROMISED,
      device,
      this,
    );
